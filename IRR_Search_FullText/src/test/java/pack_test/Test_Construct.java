...
public class Test_Construct {
	...
	private FirefoxProfile GetFireFoxProfile(int n) {
		...
		profile.setPreference("permissions.default.image", n); // не грузим
																// изображения
																// если = 2
	...
	public WebDriver GetWebDriver(int n) {
		if (driver == null) {
			...
			driver.manage().timeouts().pageLoadTimeout(120, TimeUnit.SECONDS); // время
																				// ожидания
																				// загрузки
																				// страницы(если
																				// превышено
																				// выкидывается
																				// ошибка)
			if (Proper.GetProperty("lightElement").equals("yes")) // используем
																	// ли
																	// драйвер
																	// слушатель
			{
