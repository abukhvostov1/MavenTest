...
public class Test_Irr_Search_FullTextTest extends Test_Construct {
	...
	public void Test_FindBmw(String sUrl, String sImageEnable, String sParam1,
			String sParam2) throws ExceptFailTest {
		try {
	...
	public void Test_FindBummer(String sUrl, String sImageEnable,
		...
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
					Page_IrrMain.class);
			print("\r\nПолнотекстовый поиск по марке и ее синониму и проверка совпадения найденных листингов"
					.toUpperCase());
			pageIrrMain.OpenPage(sUrl);
			pageIrrMain.CloseWindowRegion();
			pageSearch = pageIrrMain.SendTextToFieldSearch(sParam1);
			...
			print("");
			print("\r\nПроверяем совпадения результатов поиска по слову синониму для слова "
					+ sParam1);
			...
			print("Тест успешно завершен. При поиске по словам синонима \""
					+ sParam1 + "\" и \"" + sParam2
					+ "\" найдено одно и тоже объявление.");
		} finally {
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1", "sParam2" })
	public void Test_RentFlat(String sUrl, String sImageEnable, String sParam1,
			String sParam2) throws ExceptFailTest {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1", "sParam2" })
	public void Test_SuggestCompare(String sUrl, String sImageEnable,
			String sParam1, String sParam2) throws ExceptFailTest {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
					Page_IrrMain.class);
			print("\r\nСравнение саджестов для марки и ее синонима(неправильной раскладки)"
					.toUpperCase());
			pageIrrMain.OpenPage(sUrl);
			pageIrrMain.CloseWindowRegion();
	...
	public void Test_SuggestBMW(String sUrl, String sImageEnable, String sParam1)
			throws ExceptFailTest {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1" })
	public void Test_SuggestAbsentBlockInterest(String sUrl,
			String sImageEnable, String sParam1) throws ExceptFailTest,
			UnsupportedEncodingException {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
					Page_IrrMain.class);
			print("\r\nПроверка отсутствия блока \"Возможно Вам также будет интересно\""
					.toUpperCase());
			pageIrrMain.OpenPage(sUrl);
			pageIrrMain.CloseWindowRegion();
			pageIrrMain.GetListSuggest(sParam1); // получили саджесты
			pageSearch = pageIrrMain.GoToSuggest(); // переходим по первому
													// саджесту(должен быть bmw
													// x6)
			pageSearch.CheckPresentBlockInterest(1); // проверяем, что блока,
														// Вам так же будет
														// интересно, нет
			print("Тест успешно завершен.");
		} finally {
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1", "sParam2" })
	public void Test_FindBMWX5(String sUrl, String sImageEnable,
			String sParam1, String sParam2) throws ExceptFailTest,
			UnsupportedEncodingException {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
					Page_IrrMain.class);
			print("\r\nПоиск по марке и модели и проверка наличия только категории \""
					+ sParam2
					+ "\" на странице с результатами поиска".toUpperCase());
			pageIrrMain.OpenPage(sUrl);
			pageIrrMain.CloseWindowRegion();
			pageSearch = pageIrrMain.SendTextToFieldSearch(sParam1); // перешли
																		// по
																		// полнотексту
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1", "sParam2", "sParam3" })
	public void Test_FindStopWord(String sUrl, String sImageEnable,
			String sParam1, String sParam2, String sParam3)
			throws ExceptFailTest, UnsupportedEncodingException {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
					Page_IrrMain.class);
			print("\r\nПоиск по марке и модели со стоп словами и проверка категорий на странице с результатами поиска"
					.toUpperCase());
			...
			pageIrrMain.OpenPage(sUrl);
			pageIrrMain.CloseWindowRegion();
			pageSearch = pageIrrMain.SendTextToFieldSearch(sParam2); // перешли
																		// по
																		// полнотексту
			pageSearch.GetLinksMainCategoryInSeachPage();
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1", "sParam2", "sParam3" })
	public void Test_FindSinonim(String sUrl, String sImageEnable,
			String sParam1, String sParam2, String sParam3)
			throws ExceptFailTest, UnsupportedEncodingException {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
					Page_IrrMain.class);
			print("\r\nПоиск по марке и модели и синониму на русском марке и модели"
					.toUpperCase());
			...
			pageSearch.GetLinksMainCategoryInSeachPage();
			pageSearch.CheckNamesAndCountMainCategoriesInSearchPage(sParam3, 1);
			print("Получаем список найденныйх объявлений по поиску - "
					+ sParam1);
			listFirstSuggest = pageSearch.GetAdverts(); // получили список
														// ссылок на найденные
														// объявления

			print("\r\nПоиск по синониму марки и модели: ".toUpperCase()
					+ sParam2);
			pageIrrMain.OpenPage(sUrl);
			pageIrrMain.CloseWindowRegion();
			pageSearch = pageIrrMain.SendTextToFieldSearch(sParam2); // перешли
			...
			print("Получаем список найденныйх объявлений по поиску - "
					+ sParam2);
			listSecondSuggest = pageSearch.GetAdverts(); // получили список
															// ссылок на
															// найденные
															// объявления
			// ///////////////////
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1" })
	public void Test_FindSuggestCutLast(String sUrl, String sImageEnable,
			String sParam1) throws ExceptFailTest, UnsupportedEncodingException {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
					Page_IrrMain.class);
			print("\r\nПроверка отображения саджеста когда обрезка с конца"
					.toUpperCase());
	...
	@Parameters({ "sUrl", "sImageEnable", "sParam1" })
	public void Test_FindSuggest(String sUrl, String sImageEnable,
			String sParam1) throws ExceptFailTest, UnsupportedEncodingException {
		try {
			pageIrrMain = PageFactory.initElements(
					GetWebDriver(Integer.parseInt(sImageEnable)),
			...
			pageIrrMain.OpenPage(sUrl);
			pageIrrMain.CloseWindowRegion();
			ArrayList<String> suggestOnMain = pageIrrMain
					.GetListSuggest(sParam1);
			ArrayList<String> linkOnMain = pageIrrMain
					.GetListLink(sParam1);
			pageSearch = pageIrrMain.SendTextToFieldSearch(sParam1);

			pageSearch.GetLinksBlockIntresting();
			pageSearch.CompareSuggestInMainWithSuggestInBlock(suggestOnMain,
					linkOnMain);
		} finally {
			// CaptureScreenshot("FindSuggestCutLast");
			//driver.quit();
